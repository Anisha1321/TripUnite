...
// Parse newline- or comma-separated strings into arrays
const parseList = (val) => {
  if (!val) return [];
...
// Compute trip duration label from startDate / endDate strings
...
const GlowDot = ({ color = "var(--green)" }) => (
  <span style={{ width: 6, height: 6, borderRadius: "50%", background: color, boxShadow: `0 0 7px ${color}`, display: "inline-block", flexShrink: 0 }} />
);
...
const LoadingScreen = () => (
  <>
    ...
    <div style={{ minHeight: "100vh", background: "#0D1117", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      <div style={{ width: 36, height: 36, border: "2px solid #1D9E75", borderTopColor: "transparent", borderRadius: "50%", animation: "spin 0.8s linear infinite" }} />
      <p style={{ color: "#8b949e", fontSize: 14 }}>Loading trip details...</p>
    </div>
  </>
);
...
const ErrorScreen = ({ message, onBack }) => (
  <>
    <GlobalStyles />
    <div style={{ minHeight: "100vh", background: "#0D1117", display: "flex", alignItems: "center", justifyContent: "center", flexDirection: "column", gap: 16 }}>
      <Ic n="info" s={40} c="rgba(255,255,255,.2)" />
      <p style={{ color: "#8b949e", fontSize: 15 }}>{message}</p>
      <button className="btn-outline" onClick={onBack}><Ic n="back" s={14} /> Go Back</button>
    </div>
  </>
);
...
export default function TripDetails() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!snap.exists()) { setError("Trip not found."); return; }
  ...
  const tripType    = trip.tripType    || "Adventure";
  const difficulty  = trip.difficulty  || "";
  const description = trip.description || "";
  const coverUrl    = trip.coverUrl    || "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1400&q=80";
  const startDate   = formatDate(trip.startDate);
  const endDate     = formatDate(trip.endDate);
  const duration    = getDuration(trip.startDate, trip.endDate) || trip.duration || "—";
  const price       = Number(trip.price) || 0;
  const seatsTotal  = Number(trip.seats) || 12;
  const seatsLeft   = Math.max(0, seatsTotal - (joined ? 1 : 0));
  const pct         = Math.round((seatsLeft / seatsTotal) * 100);

  const inclusions   = parseList(trip.inclusions);
  const exclusions   = parseList(trip.exclusions);
  const requirements = parseList(trip.requirements);
  const itinerary    = Array.isArray(trip.itinerary) ? trip.itinerary : [];

  // Transport info
  const pickupPoint = trip.pickupPoint || "";
  const dropPoint   = trip.dropPoint   || "";
  const transport   = trip.transport   || "";

  // Host: use stored host info or fall back to creator email
  const hostName    = trip.hostName || trip.createdByEmail?.split("@")[0] || "Trip Host";
    ...
    <>
      ...
      <div className="noise" style={{ minHeight: "100vh", background: "var(--bg)" }}>
        ...
        <div style={{ position: "relative", height: "70vh", minHeight: 480, overflow: "hidden" }}>
          ...
          {/* Hero content */}
          <div style={{ position: "absolute", bottom: 0, left: 0, right: 0, padding: "0 5% 52px" }}>
            <div className="fa" style={{ marginBottom: 14, display: "flex", gap: 8, flexWrap: "wrap" }}>
              <span className="badge"><GlowDot />{tripType}</span>
              {difficulty && <span className="badge" style={{ background: "rgba(245,158,11,.1)", borderColor: "rgba(245,158,11,.3)", color: "#fbbf24" }}>{difficulty}</span>}
            </div>

            <h1 className="fa2 fraunces" style={{ fontWeight: 200, fontSize: "clamp(28px, 4.5vw, 60px)", lineHeight: 1.1, letterSpacing: "-.01em", maxWidth: 700, marginBottom: 16, color: "#fff" }}>
              {title}
            </h1>
            ...
            <div className="fa3" style={{ display: "flex", flexWrap: "wrap", alignItems: "center", gap: 20 }}>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <Ic n="map" s={14} c="var(--green-lt)" />
                <span style={{ fontSize: 14, color: "var(--dim)" }}>{destination}</span>
              </div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <Ic n="clock" s={14} c="var(--green-lt)" />
                <span style={{ fontSize: 14, color: "var(--dim)" }}>{duration}</span>
              </div>
            </div>
          </div>
          ...
          <div style={{ position: "absolute", top: 0, left: 0, right: 0, padding: "20px 5%", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 16 }}>
              <button className="btn-outline" onClick={() => navigate(-1)} style={{ padding: "8px 14px" }}>
                <Ic n="back" s={14} /> Back
              </button>
              <div className="fraunces" style={{ fontSize: 20, fontWeight: 300 }}>
                Trip<span className="grad-text">Unite</span>
        ...
        <div style={{ maxWidth: 1200, margin: "0 auto", padding: "40px 5% 80px" }}>
          <div style={{ display: "grid", gridTemplateColumns: "1fr 340px", gap: 28, alignItems: "start" }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
              ...
              {/* Stats bar */}
              <div className="card fa" style={{ padding: "20px 28px", display: "flex", justifyContent: "space-around" }}>
                <Stat label="Duration"   value={duration.split(" ")[0]} sub="days" />
                <div style={{ width: 1, background: "var(--border)" }} />
                <Stat label="Group Size" value={seatsTotal} sub="max seats" />
                <div style={{ width: 1, background: "var(--border)" }} />
                <Stat label="Start Date" value={startDate} />
                <div style={{ width: 1, background: "var(--border)" }} />
                <Stat label="End Date"   value={endDate} />
              </div>
              ...
              {description && (
                <div className="card fa2" style={{ padding: "28px 30px" }}>
                  <div className="sec-head"><Ic n="globe" s={15} c="var(--green-lt)" />Overview</div>
                  {description.split("\n\n").map((para, i) => (
                    <p key={i} style={{ fontSize: 15, color: "var(--dim)", lineHeight: 1.8, marginBottom: i < description.split("\n\n").length - 1 ? 16 : 0 }}>{para}</p>
                  ))}
                </div>
              )}
              ...
              {(pickupPoint || dropPoint || transport) && (
                <div className="card fa2" style={{ padding: "28px 30px" }}>
                  <div className="sec-head"><Ic n="truck" s={15} c="var(--green-lt)" />Transport & Logistics</div>
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(180px, 1fr))", gap: 12 }}>
                    {pickupPoint && (
                      <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px" }}>
                        <div className="label-xs" style={{ marginBottom: 6 }}>Pickup Point</div>
                        <div style={{ fontSize: 14, color: "#fff" }}>{pickupPoint}</div>
                      </div>
                    )}
                    {dropPoint && (
                      <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px" }}>
                        <div className="label-xs" style={{ marginBottom: 6 }}>Drop Point</div>
                        <div style={{ fontSize: 14, color: "#fff" }}>{dropPoint}</div>
                      </div>
                    )}
                    {transport && (
                      <div style={{ background: "rgba(255,255,255,.03)", border: "1px solid var(--border)", borderRadius: 12, padding: "14px 16px" }}>
                        <div className="label-xs" style={{ marginBottom: 6 }}>Mode of Transport</div>
                        <div style={{ fontSize: 14, color: "#fff" }}>{transport}</div>
                      </div>
                    )}
                  </div>
                </div>
                ...
                <div className="card fa3" style={{ padding: "28px 30px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 0 }}>
                    {itinerary.map((item, idx) => (
                      <div key={item.id ?? idx} style={{ display: "flex", gap: 18, position: "relative", paddingBottom: idx < itinerary.length - 1 ? 24 : 0 }}>
                        {idx < itinerary.length - 1 && (
                          <div style={{ position: "absolute", left: 14, top: 32, bottom: 0, width: 1, background: "linear-gradient(to bottom, rgba(29,158,117,.35), transparent)" }} />
                        )}
                        <div style={{ flexShrink: 0, width: 30, height: 30, borderRadius: "50%", background: "rgba(29,158,117,.12)", border: "1px solid rgba(29,158,117,.3)", display: "flex", alignItems: "center", justifyContent: "center", marginTop: 2 }}>
                          <span style={{ fontSize: 10, fontWeight: 700, color: "var(--green-lt)" }}>{idx + 1}</span>
                        </div>
                        <div style={{ flex: 1 }}>
                          {item.title && (
                          ...
                          {item.description && (
                            <p style={{ fontSize: 13.5, color: "var(--dim)", lineHeight: 1.7 }}>{item.description}</p>
                          )}
                        </div>
                      </div>
                    ))}
                ...
                <div className="card fa4" style={{ padding: "28px 30px" }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                    {requirements.map((r, i) => (
                      <div key={i} style={{ display: "flex", gap: 14, padding: "14px 16px", background: "rgba(255,255,255,.025)", border: "1px solid var(--border)", borderRadius: 12 }}>
                        <div style={{ width: 34, height: 34, borderRadius: 10, background: "rgba(29,158,117,.1)", border: "1px solid rgba(29,158,117,.2)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                          <Ic n="info" s={15} c="var(--green-lt)" />
                        </div>
                        <span style={{ fontSize: 14, color: "var(--dim)", lineHeight: 1.6, alignSelf: "center" }}>{r}</span>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* <div className="fa4">
                ...
                <div className="map-ph" style={{ height: 200, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: 10 }}>
                  <Ic n="globe" s={32} c="rgba(255,255,255,.15)" />
                  <span style={{ fontSize: 13, color: "var(--muted)" }}>Route — {destination}</span>
                  {pickupPoint && dropPoint && (
                    <span style={{ fontSize: 11, color: "rgba(255,255,255,.2)" }}>{pickupPoint} → {dropPoint}</span>
                  )}
                </div>
              </div> */}
            ...
            <div className="fa2">
              <div className="action-panel">
                ...
                <div style={{ padding: "0 24px 24px" }}>
                  ...
                  {joined && (
                    <div style={{ background: "rgba(29,158,117,.08)", border: "1px solid rgba(29,158,117,.2)", borderRadius: 10, padding: "12px 14px", marginBottom: 12, fontSize: 13, color: "var(--dim)", lineHeight: 1.6 }}>
                      🎉 You're in! The host will reach out within 24 hours to confirm details.
                    </div>
                  )}

                  <button className="btn-outline" onClick={handleShare} style={{ width: "100%", justifyContent: "center", marginBottom: 20 }}>
                    <Ic n="share" s={13} /> {shared ? "Link Copied!" : "Share with Friends"}
                  </button>
                  ...
                  <div style={{ marginBottom: 18 }}>
                    <div className="label-xs" style={{ marginBottom: 14 }}>Hosted by</div>
                    <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                      <div className="avatar">{hostInitials}</div>
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 2 }}>
                          <span style={{ fontSize: 15, fontWeight: 600, color: "#fff" }}>{hostName}</span>
                          <Ic n="verified" s={14} c="var(--green)" />
                        </div>
                        <div style={{ fontSize: 12, color: "var(--muted)" }}>Trip Host · TripUnite Verified</div>
                      </div>
                    </div>
                  </div>
