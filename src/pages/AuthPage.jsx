...
export default function AuthPage() {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-gray-50 via-stone-100 to-gray-200 px-4 py-12">
      <div className="w-full max-w-md bg-white rounded-2xl border border-black/10 px-8 py-10
                      shadow-[0_8px_40px_rgba(0,0,0,0.07)]
                      animate-[fadeSlideUp_0.5s_cubic-bezier(.22,.68,0,1.2)_both]">
        ...
        <div className="flex gap-1 bg-stone-100 rounded-xl p-1 mb-7">
          {["login", "signup"].map((tab) => (
            <button
              ...
              className={`flex-1 py-2 text-sm font-medium rounded-[10px] transition-all duration-200
                ${mode === tab ? "bg-white text-gray-900 shadow-sm" : "text-gray-400 hover:text-gray-600"}`}
            >
              {tab === "login" ? "Log in" : "Sign up"}
            </button>
          ))}
        ...
        {/* Error / Success */}
        {error && (
          <p className="text-sm text-red-500 bg-red-50 border border-red-200 rounded-xl px-4 py-2.5 mb-4">
            {error}
          </p>
        )}
        {success && (
          <p className="text-sm text-green-600 bg-green-50 border border-green-200 rounded-xl px-4 py-2.5 mb-4">
            {success}
          </p>
        )}

        {/* Form */}
        <form onSubmit={handleSubmit} className="animate-[tabSlide_0.25s_ease_both]" key={mode}>
          ...
          <button
            ...
            className="w-full flex items-center justify-center gap-2.5 py-2.5
                       border border-black/15 rounded-xl text-sm font-medium text-gray-700
                       bg-white hover:bg-stone-50 active:scale-[0.98] hover:scale-[1.02]
                       transition-all duration-200 mb-6 disabled:opacity-60 disabled:cursor-not-allowed"
          >
            <svg className="w-4 h-4" viewBox="0 0 24 24">
              <path d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" fill="#4285F4" />
              <path d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" fill="#34A853" />
